...
@router.get("/topup/{balance_id}/{dt}", status_code=status.HTTP_200_OK, response_model=models.Topup)
async def get_topup(balance_id: int, dt: date, db: Database = Depends(get_db)) -> models.Topup:
    try:
        return await services.get_topup(balance_id, dt, db=db)
